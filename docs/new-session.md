...
Gate 2–3 已过。跟手横移、双击加速、晚躲擦车、连击、五车定位（仅摩托钻缝）已锁。预览是完整一局，五车解锁已关。画面仍是色块。

下一会话按序确认：

1. Gate 4：正式 AppID + 激励广告位；真机看一次广告
2. Gate 5：好友榜默认「历史最高」；两个真实微信号对一下顺序
3. Gate 6：是否解除出图暂停、用哪套图
4. 微信助手 `run_game`：补包级验证（本会话 BLOCKED）
...
个人 IAA、动作/跑酷、12+、真夜中道路、目标平台只有微信小游戏、画布 720×1280、种子大图约 5 分钟、跟手横移、双击加速、晚躲擦车、连击阶梯 200/300/400/500、五车定位、技能条约 2 秒、分值 A、一次广告续命、只做好友榜、里程 2/6/15 局、Cocos 不用手搓 `game.js`、第一刀 A 已过线、机位修法 A、出图暂停、H5/Netlify 已下线。
