...
**正式实现状态**：S1–S5 正式代码、U1 主体与 U2 的 720×1280 核心色块矩阵已落地；混合车流、弯坡、五屏与低文字 HUD 可完整试玩。Hans 已于 2026-08-19 解除 Gate 6；出图按 `ai-game-art-pipeline` 从 A0 单锚点开始。长屏 SafeArea 先不管。微信助手 `run_game` 已通。
...
| 局结构 | 一张由种子拼成的必经大图，无圈、无分叉；道路有 OutRun 式 2.5D 左右曲率与高程 |
...
| 技能条 | 按住消耗；一次约 2 秒；满表约两次；开局半表；不用技能时空表约 20 秒回满；装置立即补满 |
| 车辆 | 跑车、小货车、摩托车、飞行汽车、压路机 |
| 车辆定位 | 跑车更快；货车更硬；摩托窄且无杠；飞行无视交通；压路机技能碾车与护栏 |
| 解锁 | 新号跑车；约 2 局货车、6 局摩托、15 局飞行汽车与压路机 |
| 里程 | 跨局累计走过的路，中途结束也累计；不是货币 |
| 续命 | 第一版不做；心归零直接结算。流量主开通后再做一次激励续命 |
| 社交 | 只做结算好友榜；不做全球榜、幻影、上榜奖励 |
| 美术 | 赛博像素；试玩机位 A-2 已锁；局内精细出图当前暂停 |

第一版明确不做：内购、金币/商城、全球榜、幻影陪跑、方向盘/摇杆、弯道离心强推车、分叉、天气物理、横幅或插屏广告、手写正式 `game.js`、复活次数购买、把旧 H5 搬进 Cocos。OutRun 式 2.5D 左右曲率与高程已于 2026-08-18 加入范围；擦车连击已由 Gate 2 改列入第一版，见 [ADR 0005](adr/0005-graze-combo.md)。
...
- 局内正式美术尚未批准；机位 A-2 已进入色块运行时，但尚未据此生产正式素材；
...
按当前个人项目的实际边界，保留下列 11 个项目技能；另有 3 个横切维护技能。技能是按需工具，不是每个会话的固定启动清单。
...
| `ai-game-art-pipeline` | 正式美术生产编排 | Hans 解除 Gate 6 后：G0 工单→G1 结构白模→G1.5 单锚点模型/提示词/参数校准→G2–G4 出图→G5 Cocos 代表资产→G6 微信包验收 | Gate 6 前生成/导入正式图；跳过风格校准直接批量；把预览图或参考图当运行时资产 |
...
暂停或不纳入本项目日常清单：`game-studio` 已卸载；`imagegen`、`design`、`tait-crt-interface-skill` 只由 `ai-game-art-pipeline` 按资产类别路由，不单独绕过 G0–G6；飞书、幻灯片、品牌、Web UI 等技能不服务当前 Cocos 微信小游戏主线。
...
`RoadPlan` 至少包含：总距离、路段顺序、每段车道宽窄、曲率、高程、主题标识、交通生成表、护盾/技能装置、天气和天色进度。曲率与高程驱动真实 2.5D 投影，但玩家仍按相对道路中心跟手横移；第一版不生成分叉、方向盘输入或离心甩车。
...
- 擦车同时覆盖迎面晚躲和同向贴近超车，两者共用连击；普通远距超车不计分，见 [ADR 0011](adr/0011-mixed-flow-graze.md)；
...
- 未释放技能时按 `200ms/s` 恢复，空表 20 秒回满；氮和飞持续释放时不边耗边回，压路机清屏与冻结结束后才开始恢复；
- 路上装置继续立即补满，承担抢节奏和连续使用的价值，不因自然恢复而删除；
...
- 玩家车保持在主镜头约 24% 画宽、80% 画高处；试玩修订后的 A-2 机位使用 `(376, 700)` 与 `k=1.572`，近处路宽与玩家锚点不变；
- 路和两侧所有平行线共用机位 A-2，不另造透视；
- 玩家纵向相对固定，世界依距离推进；路段变化只改变画面和可用车道，不变成方向盘转向；
- 天色由完成距离驱动，0% 为傍晚、100% 为清晨；暂停不推进；
- 相邻场景主题在前一段末尾 `1200` 距离内以 snapshot 连续混合；太阳和月亮在交接区重叠淡入淡出，不允许一帧互换；
- 道路外缘到画布外始终铺连续地表，Far/Mid/Near 只叠物件，任何场景不能在远景脚下至屏幕底部留下透明空带；
...
- 路段始终是必经路线，无分叉；曲率与高程连续，玩家横移始终相对道路中心；
...
**进入条件**：Hans 已于 2026-08-19 解除出图暂停。S6 仍须按 G0→G6 出图并批准可用资产。未过 G1.5 前，S1–S5 继续用统一色块，不把预览当正式局内图。

包含：

- 按 art bible 的图层栈接入天空、远景、中景、近景、连续地表承接层、道路、氮光带、车、特效、HUD；
- 机位 A-2、三车道、车位、路边推近、接地、天气滤镜和傍晚到清晨；
- 菜单/选车/续命/结算可用 CRT 表面，开车画面保持无 CRT 畸变的平面输出；
...
- 道具补满保险杠、技能条不超过上限；
- 技能空表在未释放时恰好 20 秒回满，释放期间不边耗边回，装置仍立即补满；
- 换向进入时同向车先侧向退场，迎面车从 sentinel 才激活，任一 snapshot 不同时含两种方向；
- 场景主题在边界前连续混合，段界不改变道路几何与固定 seed；
...
- 没有重新引入圈、分叉、方向盘/摇杆、弯道离心强推车、全球榜、内购或强制广告；擦车连击与 2.5D 曲率/高程为已接受变更；
...
| R7 | 正式美术未批准导致 S6 阻塞 | 高 / 中 | G0 未写或 G1.5 未过就批量；预览当运行时资产 | S1–S5 保持统一色块；不偷用预览 | Hans / Art |
...
- 未填 G0 / 未过 G1.5 却批量出图，或把预览/签名稿当正式资产；
- 连续两个检查点没有新的可运行行为或证据。
